...
- /var    - variable data 
- /proc -       - All changes to files in /proc/ are immediately recognized by the kernel.
...
-  The bootloader - system boot cycle, what is the program that is run after the BIOS completes  - On all connected storage media in the defined boot device order.
...
-  cat  /boot/grub/grub.cfg - show grub boot config 


- /sbin/grub-install - Install GRUB on your drive. - 
...
- rpm -iv iotop - install 
- rpm -qi iotop - get all information 
...
- hyperviros type 1 - only with host system withouthost operation systemem
- hyperviros type 1 - with operation systemem
