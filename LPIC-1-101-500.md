...
- /boot   - The boot partition contains files used by the bootloader to load the operating system -  config-6.8.0-41-generic - a configuration file for the Linux kernel
- /etc    - configuration files (system & app)
- /home   - Home directory to store personal files and preferences, and most of them are located under /home
- /media  - removable media 
- /mnt    - removable media  (old location)
- /opt    - third-party apps 
- /tmp    - temporary files   - all filers removed after restart
...
-  sort - sort lines in file 
  - sort -n filename - sort lines numerically (not print string value )
- paste - copy file to another as second column 
  - paste from_file to_file - 
- split -l 2 - split file by 2 lines 
...
- grep - find something in file 
  ...
  - grep root -d skip   /etc/* - grep "root" in all files only in etc folder
  - grep root -d recurse /etc/* - grep  recursively "root" in all folders/files in etc folder
  - grep -c root /etc/passwd - show how many times you have "root" in file - grep -c root -d skip  /etc/*
